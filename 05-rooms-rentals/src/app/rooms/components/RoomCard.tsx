...
export default function RoomCard({ room }: { room: Room }) {
  return (
    <Link href={`/rooms/${room.id}`} className="border rounded-lg p-4 hover:shadow-md transition-shadow block">
      ...
      {room.owner && (
        <div className="mt-2 text-sm text-gray-500">
          ...
          <img src={room.owner.portraitUrl || "/placeholder.svg"} className="rounded-full w-12 h-12 mt-2" alt="Owner" />
        </div>
      )}
    </Link>
  )
